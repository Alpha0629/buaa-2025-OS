...
void schedule(int yield) {
	...
	int left = 0;
	struct Env * best = NULL; //LIST_FIRST(&env_edf_sched_list);
	LIST_FOREACH (env, &env_edf_sched_list, env_edf_sched_link) {
		if (env->env_runtime_left != 0){
			left = 1;
		}
		if( best == NULL && env->env_runtime_left>0){
			best = env;
		}
		if (env->env_runtime_left > 0 && env->env_period_deadline < best->env_period_deadline) {
			best = env;
		} else if (env->env_runtime_left > 0 && env->env_period_deadline == best->env_period_deadline) {
			if (env->env_id < best->env_id){
				best = env;
			}
		}
	}
